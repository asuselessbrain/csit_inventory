...
import { Calendar, FileText, Download, CheckCircle2, Clock, AlertCircle } from "lucide-react";
                ...
                <section className="space-y-4">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">
                            {proposal.data.tasks.length} {proposal.data.tasks.length === 1 ? "task" : "tasks"} assigned to this proposal
                        </p>
                    </div>
                    ...
                    <div className="grid gap-4">
                        {proposal.data.tasks.map((task: ITask, idx: number) => {
                            ...
                            return (
                                <Card key={idx} className="hover:shadow-lg transition-shadow border-slate-200 dark:border-slate-700">
                                    <CardHeader className="pb-3">
                                        <div className="flex items-start justify-between gap-4">
                                            ...
                                            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium whitespace-nowrap ${getStatusColor(task.status)}`}>
                                        ...
                                        {task.requirements && task.requirements.length > 0 && (
                                            <div className="space-y-2">
                                                ...
                                                <ul className="space-y-1 ml-7">
                                                    {task.requirements.map((req: string, ridx: number) => (
                                        ...
                                        {task.referenceMaterials && task.referenceMaterials.length > 0 && (
                                            <div className="space-y-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="font-medium text-gray-900 dark:text-white">Reference Materials</span>
                                                </div>
                                                <div className="flex flex-wrap gap-2 ml-7">
                                                    {task.referenceMaterials.map((url: string, midx: number) => (
                                                        <Button
                                                            key={midx}
                                                            asChild
                                                            variant="outline"
