...
export default function AssignCourseTeacher({
  ...
  const handleAssignTeacher = async () => {
    ...
    if (!selectedTeacher || !course) {
      toast.warning("Please select a course teacher!", { id: toastId });
      setIsLoading(false);
      return;
    }
    const data = {
    ...
    if (res.success) {
      toast.success(res.message || "Course Teacher assign successful", {
        id: toastId,
      });
    }
    if (!res.success) {
      toast.error(res.errorMessage || "Failed to assign course teacher", {
        id: toastId,
      });
    }
    setIsLoading(false);
