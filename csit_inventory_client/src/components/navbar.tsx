            ...
            <div className="max-w-360 mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-16">
                    ...
                    <div className="hidden lg:flex items-center gap-1">
                        {navLinks.map((link) => (
                            <Link
                                key={link.href}
                                href={link.href}
                                className="px-4 py-2 text-slate-700 dark:text-slate-300 font-medium text-sm transition-all duration-200 relative group"
                            >
                    ...
                    <div className="flex items-center gap-3 sm:gap-4">
                        ...
                        {
                            user?.user ? <div className="hidden md:block relative">
                            <button
                            ...
                            {isProfileOpen && (
                                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-xl dark:shadow-2xl border border-slate-200 dark:border-slate-700 py-2 animate-in fade-in slide-in-from-top-2 duration-200">
                                    ...
                                    <div className="border-t border-slate-200 dark:border-slate-700 pt-2">
                                        <button className="w-full flex items-center gap-3 px-4 py-2.5 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors duration-150 group">
                        ...
                        </div> : <Button asChild><Link href="/login">Login</Link></Button>
                        }
                ...
                {isOpen && (
                    <div className="lg:hidden pb-4 space-y-1 border-t border-slate-200 dark:border-slate-800 pt-4 animate-in fade-in slide-in-from-top-2 duration-200">
                        ...
                        <Link href="/profile" className="block px-4 py-2.5 text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg font-medium text-sm transition-colors duration-150">
                            My Profile
                        </Link>
                        <button className="w-full text-left px-4 py-2.5 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg font-medium text-sm transition-colors duration-150">
                            Sign Out
                        </button>
                    </div>
                )}
